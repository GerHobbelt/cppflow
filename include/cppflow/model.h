...
namespace cppflow
{
    ...
    inline model::model(const std::string &filename, bool frozen)
    {
        ...
        std::shared_ptr<TF_ImportGraphDefOptions> graph_opts = {TF_NewImportGraphDefOptions(), TF_DeleteImportGraphDefOptions};
        TF_GraphImportGraphDef(this->graph.get(), buffer.get(), graph_opts.get(), context::get_status());
    ...
    {
        return (*this)({{"serving_default_input", input}}, {"StatefulPartitionedCall"})[0];
    }
}
